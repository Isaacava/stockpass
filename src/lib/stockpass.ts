...
export async function ensureProfile(wallet: string) {
  const { error } = await supabase.from('stockpass_profiles').upsert(
...
export async function ensureAsset(asset: StockAsset) {
  ...
  const { error } = await supabase.from('stockpass_assets').upsert(
    {
      mint: asset.mint,
      symbol: asset.symbol,
      name: asset.name,
...
export async function saveVerificationSnapshot(wallet: string, position: VerifiedPosition, slot: number) {
  ...
  const { data, error } = await supabase.from('stockpass_verification_snapshots').insert({
    wallet,
    mint: position.mint,
...
export async function publishVerifiedPost(input: {
  wallet: string;
  ...
  const { data, error } = await supabase.from('stockpass_posts').insert({
    wallet: input.wallet,
    mint: input.position.mint,
    body: input.body.trim(),
    proof_type: 'verified_holder',
    verification_snapshot_id: snapshotId
  ...
  await supabase.from('stockpass_activity_events').insert({
    ...
    metadata: { proof_type: 'verified_holder', verification_snapshot_id: snapshotId },
...
export async function loadAlerts(wallet: string) {
  const { data, error } = await supabase
    ...
    .select('id, mint, direction, target_price, active')
    .eq('wallet', wallet)
    .order('created_at', { ascending: false });
  if (error) throw error;
  return data ?? [];
