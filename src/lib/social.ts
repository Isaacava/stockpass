...
export async function loadProfile(wallet: string) {
  const { data, error } = await supabase
    .from('stockpass_profiles')
    .select('wallet, handle, display_name, bio, avatar_url, created_at, updated_at')
    .eq('wallet', wallet)
    .maybeSingle();
...
export async function saveProfile(input: Pick<StockPassProfile, 'wallet'> & Partial<Omit<StockPassProfile, 'wallet' | 'created_at' | 'updated_at'>>) {
  const { data, error } = await supabase
    .from('stockpass_profiles')
    .upsert({ ...input, updated_at: new Date().toISOString() }, { onConflict: 'wallet' })
    .select('wallet, handle, display_name, bio, avatar_url, created_at, updated_at')
    .single();
  if (error) throw error;
...
export async function followWallet(followerWallet: string, followedWallet: string) {
  ...
  const { error } = await supabase.from('stockpass_follows').upsert(
    { follower_wallet: followerWallet, followed_wallet: followedWallet },
    { onConflict: 'follower_wallet,followed_wallet', ignoreDuplicates: true }
  );
  if (error) throw error;
}
...
export async function loadNotifications(wallet: string) {
  const { data, error } = await supabase
    .from('stockpass_notifications')
    .select('id, actor_wallet, event_type, post_id, asset_mint, message, read_at, created_at')
    .eq('recipient_wallet', wallet)
    .order('created_at', { ascending: false })
    .limit(40);
  if (error) throw error;
  return (data ?? []) as StockPassNotification[];
}
...
export async function fanoutPostNotification(post: FeedPost) {
  const { data: followers, error } = await supabase
    .from('stockpass_follows')
    .select('follower_wallet')
    .eq('followed_wallet', post.wallet);
  if (error) throw error;
  if (!followers?.length) return;

  const message = `${shortWallet(post.wallet)} published a verified StockPass position${post.mint ? '.' : '.'}`;
  const rows = followers
    .map((row) => row.follower_wallet)
    .filter((wallet): wallet is string => Boolean(wallet) && wallet !== post.wallet)
    .map((recipient_wallet) => ({
      recipient_wallet,
      actor_wallet: post.wallet,
      event_type: 'verified_post',
      post_id: post.id,
      asset_mint: post.mint,
      message
    }));
...
export async function loadRecentPostsByWallet(wallet: string, limit = 12) {
  const { data, error } = await supabase
    .from('stockpass_posts')
    .select('id, wallet, mint, body, proof_type, created_at')
    .eq('wallet', wallet)
    .order('created_at', { ascending: false })
    .limit(limit);
  if (error) throw error;
...
export async function searchProfiles(query: string, limit = 10) {
  ...
  const { data, error } = await supabase
    .from('stockpass_profiles')
    .select('wallet, handle, display_name, bio, avatar_url, created_at, updated_at')
    .or(`handle.ilike.%${needle}%,display_name.ilike.%${needle}%,wallet.ilike.%${needle}%`)
    .limit(limit);
  if (error) throw error;
  return (data ?? []) as StockPassProfile[];
}
