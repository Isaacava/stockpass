...
export default function WggDashboard({
  ...
  return (
    <section className="sp-dashboard">
      ...
      <section className="sp-positions">
        <div className="sp-section-head">
          <div>
            <div className="sp-overline"><span /> LIVE POSITIONS</div>
            <h2>Your Kamino xStock obligations</h2>
          </div>
          <div className="sp-section-meta">{lastLoaded ? `Synced ${lastLoaded.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'Not scanned'} · read only</div>
        </div>

        {loading ? (
          <div className="sp-table-state"><RefreshCw size={18} className="wgg-spin" /><strong>Scanning mainnet positions</strong><span>Reading Kamino obligations and market context.</span></div>
        ) : positions.length === 0 ? (
          <div className="sp-table-state sp-table-empty">
            <Layers3 size={19} />
            <strong>No xStock-backed Kamino position found</strong>
            <span>This is a real-data empty state. Nothing is fabricated to make the dashboard look populated.</span>
            <button className="sp-secondary" onClick={() => void scan()}>Scan again</button>
          </div>
        ) : (
          <div className="sp-position-table">
            <div className="sp-position-table-head"><span>Position</span><span>Collateral</span><span>Current / stressed LTV</span><span>Status</span><span>Action</span></div>
            {positions.map((position) => {
              const positionRows = rows.filter((row) => row.position.obligation === position.obligation);
              const lead = positionRows[0];
              const risk = positionRows.reduce<ReturnType<typeof evaluateWeekendRisk> | null>((acc, row) => {
                if (!row.risk) return acc;
                if (!acc) return row.risk;
                const rank: Record<string, number> = { flagged: 3, watch: 2, safe: 1 };
                return rank[row.risk.status] > rank[acc.status] ? row.risk : acc;
              }, null);

              return (
                <article className="sp-position-item" key={position.obligation}>
                  <div className="sp-position-identity">
                    <div className="sp-asset-pile">
                      {positionRows.slice(0, 3).map((row) => <span key={row.stock.mint}>{row.symbol.slice(0, 3)}</span>)}
                    </div>
                    <div>
                      <strong>{positionRows.map((row) => row.symbol).join(' · ') || 'xStock collateral'}</strong>
                      <span>{shortAddress(position.obligation)} · {positionRows.length} asset{positionRows.length === 1 ? '' : 's'}</span>
                    </div>
                  </div>

                  <div className="sp-position-value">
                    <strong>{money(position.depositValueUsd)}</strong>
                    <span>{positionRows.map((row) => row.price ? `${row.symbol} ${money(row.price.price)}` : `${row.symbol} price unavailable`).join(' · ')}</span>
                  </div>

                  <div className="sp-position-ltv">
                    <strong>{pct(position.ltvPct)} <span>→</span> {risk ? pct(risk.stressedLtvPct) : '—'}</strong>
                    <span>Liquidation {pct(position.liquidationLtvPct)}</span>
                  </div>

                  <div className={`sp-position-status sp-risk-${risk?.status ?? 'empty'}`}>
                    {risk?.status === 'flagged' ? <ShieldAlert size={14} /> : risk?.status === 'watch' ? <TrendingDown size={14} /> : risk?.status === 'safe' ? <ShieldCheck size={14} /> : <Activity size={14} />}
                    <span>{risk?.status?.toUpperCase() ?? 'PENDING'}</span>
                    {lead?.gap?.sampleCount != null && <small>{lead.gap.sampleCount} gap samples</small>}
                  </div>

                  <div className="sp-position-action">
                    {risk?.status === 'flagged' && lead ? (
                      <>
                        <button className="sp-secondary" onClick={() => void prepareFix(lead, 'deposit')} disabled={preparing || authenticating}>Protect</button>
                        <button className="sp-quiet-button" onClick={() => document.getElementById('sp-method')?.scrollIntoView({ behavior: 'smooth' })}>Why?</button>
                      </>
                    ) : (
                      <button className="sp-quiet-button" onClick={() => document.getElementById('sp-method')?.scrollIntoView({ behavior: 'smooth' })}>Inspect</button>
                    )}
                  </div>
                </article>
              );
      ...
      <section className="sp-market-strip">
        <div className="sp-market-strip-title">
          <div className="sp-overline"><span /> MARKET CONTEXT</div>
          <strong>xStocks + weekend history</strong>
          <span>Current prices are read from xStocks. Historical gap stats are calculated separately; the market model never fabricates a weekend observation.</span>
        </div>
        <div className="sp-market-items">
      ...
      {signature && (
        <div className="sp-alert sp-alert-success">
          ...
          <div><strong>Transaction confirmed</strong><span>{signature}</span></div>
        </div>
      )}
