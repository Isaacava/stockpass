...
export default function App() {
  ...
  }, [address, refreshMarketData, verifyWallet]);
  ...
  const addAlert = async (symbol?: string) => {
    ...
    const current = targetAsset ? prices[targetAsset.symbol] : undefined;
    ...
    const mint = targetAsset.mint;
    try {
      await createPriceAlert({ wallet: address, mint, direction: 'above', target_price: target });
      const nextAlert: AlertRow = { id: crypto.randomUUID(), symbol: targetAsset.symbol, direction: 'above', target, active: true, mint };
  ...
  return (
    <div className="app-shell">
      ...
      <div className="main">
        ...
        <main>
          <section className="hero"><div className="hero-copy-wrap"><p className="eyebrow">ONCHAIN SOCIAL TRADING</p><h1>Show the position.<br /><span>Prove the position.</span></h1><p className="hero-copy">Follow conviction backed by wallet state, not screenshots. StockPass links every position post to what existed on Solana at the moment it was published.</p><div className="hero-actions"><button className="primary-btn" onClick={() => setComposeOpen(true)} disabled={!positions.length}><Plus size={15} /> Post a position</button><button className="ghost-btn" onClick={() => nav('portfolio')}><WalletCards size={15} /> View portfolio</button></div><div className="hero-proof"><div><strong>OWNERSHIP</strong><span>Verified from the connected wallet.</span></div><div><strong>MARKET DATA</strong><span>Official xStocks public feed.</span></div><div><strong>PROOF</strong><span>Snapshot timestamp and slot.</span></div></div></div><div className="hero-card"><div className="hero-card-top"><span>YOUR VERIFICATION</span><ShieldCheck size={14} /></div><div className="score-line"><strong>{positions.length}</strong><span>live proofs</span><div className="score-ring">✓</div></div><div className="trust-row"><span>Wallet</span><b>{address ? shortAddress(address) : '—'}</b></div><div className="trust-row"><span>Mainnet positions</span><b>{positions.length}</b></div><div className="trust-row"><span>Data</span><b>{loadingData || checking ? 'Checking' : 'Live'}</b></div></div></section>
