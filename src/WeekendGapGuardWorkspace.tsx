...
import { evaluateWeekendRisk, calculateCollateralUsdForTargetLtv } from './lib/wggRisk';
import { prepareDepositProtectionAction } from './lib/kaminoProtection';
...
export default function WeekendGapGuardWorkspace() {
  ...
  const [preparingProtection, setPreparingProtection] = useState(false);
  const [prepared, setPrepared] = useState<PreparedState>(null);
  ...
  const loadPositions = async () => {
    if (!address) return;
    setMonitoring(true); setLoadingPositions(true); setLoadError(''); setPythError(''); setGapError(''); setProtectionError(''); setPrepared(null);
    try {
      ...
      setWeekendGaps({});
      if (symbols.length) {
        try { setPythPrices(await fetchPythPrices(symbols)); } catch (error) { setPythPrices({}); setPythError(error instanceof Error ? error.message : 'Pyth live pricing is not configured yet.'); }
        try { setWeekendGaps(await fetchWeekendGapSummaries(symbols, 13)); } catch (error) { setWeekendGaps({}); setGapError(error instanceof Error ? error.message : 'Historical weekend-gap data is not configured yet.'); }
      } else setPythPrices({});
      setLastLoaded(new Date());
    } catch (error) {
      setPositions([]); setPythPrices({}); setWeekendGaps({}); setLoadError(error instanceof Error ? error.message : 'Could not load Kamino obligations.');
  ...
  const prepareProtection = async (position: KaminoXStockPosition, stockSymbol: string) => {
    ...
    try {
      const action = await prepareDepositProtectionAction({ wallet: address, rpcUrl: endpoint, obligationAddress: position.obligation, reserveAddress: item.reserve, amountBaseUnits: baseUnits });
      setPrepared({ obligation: position.obligation, symbol: stockSymbol, kind: 'deposit', amountBaseUnits: baseUnits, instructionCount: action.instructionCount });
    } catch (error) {
      setProtectionError(error instanceof Error ? error.message : 'Could not prepare the Kamino protection action.');
    } finally { setPreparingProtection(false); }
  };

  return <div className="wgg-app">
    <header className="wgg-header"><div className="wgg-brand"><span className="wgg-mark">WG</span><div><strong>Weekend Gap Guard</strong><small>risk protection for xStock collateral</small></div></div><div className="wgg-header-right"><span className="wgg-mainnet"><i /> SOLANA MAINNET</span>{isConnected ? <div className="wgg-wallet"><Wallet size={14} />{address ? `${address.slice(0, 4)}…${address.slice(-4)}` : 'Connected'}</div> : <button className="wgg-connect" onClick={connect}>Connect wallet</button>}</div></header>
    <main className="wgg-main">
      <section className="wgg-hero"><div className="wgg-hero-copy"><div className="wgg-eyebrow"><span /> WEEKEND RISK MONITOR</div><h1>Know the gap<br /><em>before Monday.</em></h1><p>Weekend Gap Guard watches real Kamino xStock collateral and checks whether the current liquidation buffer can absorb a modeled historical weekend move.</p><div className="wgg-hero-actions">{!isConnected ? <button className="wgg-primary" onClick={connect}>Connect wallet <ArrowRight size={15} /></button> : <button className="wgg-primary" onClick={() => void loadPositions()} disabled={loadingPositions}>{loadingPositions ? <><LoaderCircle size={15} className="wgg-spin" /> Scanning Kamino</> : <>Scan my Kamino positions <ArrowRight size={15} /></>}</button>}<span className="wgg-hero-note"><ShieldCheck size={14} /> Read-only scan · every future fix is user-signed</span></div></div><div className="wgg-hero-card"><div className="wgg-card-top"><span>FRIDAY CHECK</span><span className="wgg-status-dot"><i /> {loadingPositions ? 'SCANNING' : isConnected ? 'READY' : 'WAITING'}</span></div><div className="wgg-risk-meter"><div style={{ width: `${isConnected ? Math.min(100, Math.max(8, 100 - riskCounts.flagged * 25 - riskCounts.watch * 10)) : 12}%` }} /></div><div className="wgg-meter-label"><span>Protection signal</span><strong>{riskCounts.flagged ? `${riskCounts.flagged} flagged` : riskCounts.watch ? `${riskCounts.watch} on watch` : summary.positions.length ? 'Gap model loaded' : 'Waiting for scan'}</strong></div><div className="wgg-card-rule"><span>xStock positions</span><b>{summary.positions.length || '—'}</b></div><div className="wgg-card-rule"><span>Collateral value</span><b>{summary.collateralUsd ? `$${summary.collateralUsd.toLocaleString(undefined, { maximumFractionDigits: 2 })}` : '—'}</b></div><div className="wgg-card-rule"><span>Debt value</span><b>{summary.debtUsd ? `$${summary.debtUsd.toLocaleString(undefined, { maximumFractionDigits: 2 })}` : '—'}</b></div></div></section>
      <section className="wgg-principles"><div><Gauge size={18} /><div><strong>Independent Pyth signal</strong><span>Live Pyth price is displayed beside Kamino's position state.</span></div></div><div><Bell size={18} /><div><strong>Historical gap check</strong><span>A 13-week Friday-close → next-session-open history is compared with the live liquidation buffer.</span></div></div><div><ShieldCheck size={18} /><div><strong>User-signed protection</strong><span>When the model flags a position, WGG prepares a specific Kamino action but never signs or sends it.</span></div></div></section>
      {isConnected && <section className="wgg-dashboard"><div className="wgg-section-head"><div><div className="wgg-eyebrow">REAL KAMINO + PYTH DATA</div><h2>{monitoring ? 'Your xStock-backed obligations.' : 'Scan your Kamino exposure.'}</h2><p>{lastLoaded ? `Mainnet scan completed ${lastLoaded.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.` : 'This reads the current Kamino Main Market, filters official xStocks, and requests live Pyth plus historical weekend-gap data.'}</p></div><button className="wgg-secondary" onClick={() => void loadPositions()} disabled={loadingPositions}><RefreshCw size={14} /> Refresh</button></div>
        ...
        {protectionError && <div className="wgg-error"><AlertTriangle size={18} /><div><strong>Protection preparation failed</strong><span>{protectionError}</span></div></div>}
