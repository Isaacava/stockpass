...
type PreparedProtection = { symbol: string; amountBaseUnits: string; instructionCount: number } | null;
type PreparedResponse = { instructions?: unknown[] };
...
export default function WeekendGapGuardWorkspace() {
  ...
  async function scan() {
    ...
    try {
      ...
      if (!symbols.length) { setPythPrices({}); setWeekendGaps({}); }
      else {
  ...
  async function prepareFix(row: (typeof rows)[number]) {
    ...
    try {
      ...
      const response = data as PreparedResponse;
      setPrepared({ symbol: row.symbol, amountBaseUnits, instructionCount: response.instructions?.length ?? 0 });
    } catch (e) {
