...
function Composer({ positions, onClose, onPublish }: { positions: VerifiedPosition[]; onClose: () => void; onPublish: (position: VerifiedPosition, body: string) => Promise<void> }) {
  ...
  return <div className="sp3-overlay" onClick={onClose}><section className="sp3-composer" onClick={(event) => event.stopPropagation()}><div className="sp3-composer-head"><div><div className="sp3-kicker">PROOF POST</div><h2>Share a verified position</h2></div><button onClick={onClose} aria-label="Close"><X size={17} /></button></div>{current ? <><div className="sp3-composer-asset"><span className="sp3-holding-symbol">{current.symbol.replace(/x$/i, '').slice(0, 4)}</span><div><strong>{current.symbol}</strong><span>{current.balance.toLocaleString()} held · verified now</span></div><ShieldCheck size={16} /></div>{positions.length > 1 && <select value={selected} onChange={(event) => setSelected(event.target.value)}>{positions.map((position) => <option key={position.mint} value={position.mint}>{position.symbol}</option>)}</select>}<textarea value={body} onChange={(event) => setBody(event.target.value)} maxLength={280} placeholder="What are you seeing? Keep the post useful; StockPass attaches the proof." /><div className="sp3-composer-foot"><span>{body.length}/280 · snapshot will be taken from Solana mainnet</span><button className="sp3-primary" disabled={!body.trim()} onClick={() => void onPublish(current, body.trim())}><ShieldCheck size={14} /> Publish verified post</button></div></> : <div className="sp3-empty"><ShieldCheck size={20} /><strong>You need a supported xStock</strong><span>Hold a verified xStock in this wallet before publishing a position proof.</span></div>}</section></div>;
}
